...
{
	float ent_gravity;
	//jc429 mod gravity so jumps actually do shit in HL2
	if (player->GetGravity())
		ent_gravity = 0.2*player->GetGravity();
	else
		ent_gravity = 0.2*1.0;
...
void CGameMovement::SetGroundEntity( trace_t *pm )
{
	...
	if ( newGround )
	{
		...
		CheckJumpButton(); //jc429 makes the player jump continuously
	}
}
